...
ATENÃ‡ÃƒO: este setup usa apenas Docker Compose para gerar os dados (serviÃ§o `data-generator`).
...
```bash
docker compose exec postgres psql -U nextage nextage_db -c 'SELECT COUNT(*) FROM sales;'
