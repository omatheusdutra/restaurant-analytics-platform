...
Esse documento descreve a característica de geração de dados adotada pelo script tools/data-generator/generate_data.py
...
-- Catálogo
products (id, brand_id, category_id, name, cost_price)
...
channels (id, name, type, commission_pct)  -- 'P' presencial, 'D' delivery
...
```bash
python tools/data-generator/generate_data.py \
    --months 6 \
    --stores 50 \
    --products 500 \
    --items 200 \
