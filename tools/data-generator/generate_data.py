#!/usr/bin/env python3
"""
God Level Coder - Data Generator
...
def setup_base_data(conn):
    ...
    for name, ch_type, weight, commission in CHANNELS:
        # Normalize possible 'App Próprio' to 'aiqfome'
        norm = unicodedata.normalize('NFD', name).encode('ascii', 'ignore').decode('utf-8').lower()
        if norm == 'app proprio':
            name = 'aiqfome'
        cursor.execute("""
            INSERT INTO channels (brand_id, name, description, type, commission_pct)
            VALUES (%s, %s, %s, %s, %s) RETURNING id
        """, (brand_id, name, f'Canal {name}', ch_type, commission))
        channel_ids.append({
...
def generate_products_and_items(conn, sub_brand_ids, num_products=500, num_items=200):
    ...
    # Product categories (distribute exactly num_products across categories)
    num_prod_cats = len(CATEGORIES_PRODUCTS)
    base_per_cat = num_products // num_prod_cats
    extra = num_products % num_prod_cats

    for idx, cat_name in enumerate(CATEGORIES_PRODUCTS):
        cursor.execute("""
        ...
        products_to_create = base_per_cat + (1 if idx < extra else 0)
        ...
        for i in range(products_to_create):
            ...
            # Compute a plausible cost price (60-75% of an inferred base price)
            base_price = round(random.uniform(15, 120), 2)
            cost_price = round(base_price * random.uniform(0.6, 0.75), 2)
            cursor.execute("""
                INSERT INTO products (brand_id, sub_brand_id, category_id, name, pos_uuid, cost_price)
                VALUES (%s, %s, %s, %s, %s, %s) RETURNING id
            """, (BRAND_ID, sub_brand_id, cat_id, name, f"prod_{cat_id}_{i}", cost_price))
    ...
    # Item categories (for complements/additions) — distribute exactly num_items
    num_item_cats = len(CATEGORIES_ITEMS)
    base_items_per_cat = num_items // num_item_cats
    extra_items = num_items % num_item_cats

    for idx, cat_name in enumerate(CATEGORIES_ITEMS):
        cursor.execute("""
            INSERT INTO categories (brand_id, name, type)
        ...
        target_in_cat = base_items_per_cat + (1 if idx < extra_items else 0)
        created_in_cat = 0

        # Prefer realistic names up to the target
        if item_names_list:
            for item_name in item_names_list[:target_in_cat]:
                sub_brand_id = random.choice(sub_brand_ids)
        ...
        # If still short, fill with numbered items until we hit the target
        while created_in_cat < target_in_cat:
            sub_brand_id = random.choice(sub_brand_ids)
            name = f"{cat_name[:-1]} #{created_in_cat+1:02d}"

            cursor.execute("""
                INSERT INTO items (brand_id, sub_brand_id, category_id, name, pos_uuid)
                VALUES (%s, %s, %s, %s, %s) RETURNING id
            """, (BRAND_ID, sub_brand_id, cat_id, name, f"item_{cat_id}_{created_in_cat}"))
...
def generate_customers(conn, num_customers=10000):
    ...
    for i in range(num_customers):
        name = fake.name()
        email = fake.email()
        # Ensure a valid, non-empty email for data quality checks
        if not email or '@' not in email:
            base = unicodedata.normalize('NFD', name).encode('ascii', 'ignore').decode('utf-8').lower()
            base = re.sub(r'[^a-z0-9]+', '.', base).strip('.')
            if not base:
                base = f"cliente{i+1}"
            email = f"{base}.{i+1}@example.com"
        batch.append((
            name, email.lower(), fake.phone_number(), fake.cpf(),
...
def generate_sales(
    ...
    months=6,
    monthly_growth=None,
    growth_store_id=None,
    seasonal_months_arg=None,
    seasonal_products_count=None,
    seed=None,
):
    ...
    # Monthly growth (2–3%) and monthly noise (±10%)
    base_monthly_growth = monthly_growth if monthly_growth is not None else random.uniform(0.02, 0.03)
    month_noise = {}

    # Choose a growth store (+5%/month)
    growth_store_id = growth_store_id if growth_store_id else (random.choice(stores) if stores else None)

    # Seasonal products: +80% in two random months
    if seasonal_months_arg:
        seasonal_months = set([int(m) for m in str(seasonal_months_arg).split(',') if m.strip().isdigit()])
        if not seasonal_months:
            seasonal_months = set(random.sample(list(range(1, 13)), k=2))
    else:
        seasonal_months = set(random.sample(list(range(1, 13)), k=2))

    pick_count = seasonal_products_count if seasonal_products_count else max(1, min(10, len(products)//20)) if products else 0
    seasonal_product_ids = set([p['id'] for p in random.sample(products, k=pick_count)]) if products and pick_count else set()

    # Precompute product weights per month (seasonality)
    weights_by_month = {}
    for m in range(1, 13):
        mult = 1.8 if m in seasonal_months else 1.0
        weights_by_month[m] = [ (p['popularity'] * (mult if p['id'] in seasonal_product_ids else 1.0)) for p in products ]
    ...
    total_sales = 0
    # Larger batches reduce round-trips and COMMITs significantly
    batch_size = 8000
    # Precompute hour weights once (hot loop optimization)
    precomputed_hour_weights = [get_hour_weight(h) * 100 for h in range(24)]
    
    while current_date <= end_date:
        weekday = current_date.weekday()
        day_mult = WEEKDAY_MULT[weekday]
        # Apply monthly growth + noise per month
        months_since_start = (current_date.year - start_date.year) * 12 + (current_date.month - start_date.month)
        growth_factor = (1.0 + base_monthly_growth) ** max(0, months_since_start)
        month_key = current_date.strftime('%Y-%m')
        if month_key not in month_noise:
            month_noise[month_key] = 1.0 + random.uniform(-0.10, 0.10)
        day_mult *= growth_factor * month_noise[month_key]
        ...
        for _ in range(daily_sales):
            # Hour distribution
            hour = random.choices(range(24), weights=precomputed_hour_weights)[0]
            ...
            # Select entities
            # Weight one store to grow +5% per month
            if stores:
                store_weights = [ ((1.0 + 0.05) ** max(0, months_since_start)) if s == growth_store_id else 1.0 for s in stores ]
...
def generate_single_sale(sale_time, store_id, channel, customer_id, products, items, option_groups, weights_by_month=None):
    """Generate a single sale with all related data"""
    
    # Select 1-5 products
    num_products = min(5, max(1, int(random.expovariate(0.5)) + 1))
    # Use month-aware weights for seasonality if provided
    month = sale_time.month
    w = None
    if weights_by_month and month in weights_by_month:
        w = weights_by_month[month]
    else:
        w = [p['popularity'] for p in products]
    selected_products = random.choices(products, weights=w, k=num_products)
...
def insert_sales_batch(cursor, sales_batch, items, option_groups):
    ...
    # Cache payment type ids to avoid querying per payment
    cursor.execute("SELECT id, description FROM payment_types")
    pay_types = {row[1]: row[0] for row in cursor.fetchall()}
    ...
    payments_rows = []  # batch payments at the end for speed
    for sale_id, sale in zip(sale_ids, sales_batch):
        ...
        # Collect payments to insert in one batch later
        for payment in sale['payments']:
            pt_id = pay_types.get(payment['type'])
            if pt_id:
                payments_rows.append((sale_id, pt_id, Decimal(str(payment['value']))))

    if payments_rows:
        execute_batch(cursor, """
            INSERT INTO payments (sale_id, payment_type_id, value)
            VALUES (%s,%s,%s)
        """, payments_rows, page_size=1000)
...
def main():
    ...
    parser.add_argument('--months', type=int, default=7, help='Months of sales data')
    # Advanced behavior toggles
    parser.add_argument('--monthly-growth', type=float, default=None,
                       help='Monthly growth factor (e.g., 0.025 for 2.5%%). Default: random between 0.02–0.03')
    parser.add_argument('--growth-store-id', type=int, default=None,
                       help='Specific store id that should grow 5%% per month (default: random)')
    parser.add_argument('--seasonal-months', type=str, default=None,
                       help='Comma-separated months for seasonality boost (e.g., 1,7). Default: 2 random months')
    parser.add_argument('--seasonal-products', type=int, default=None,
                       help='How many products are seasonal (default: ~5%% capped at 10)')
    ...
    print("=" * 70)
    print("God Level Coder - Data Generator")
    print("=" * 70)
    print(f"Generating {args.months} months of restaurant operational data...")
    print()
    
    conn = get_db_connection(args.db_url)
    # Speed up bulk inserts for this session (durability acceptable for synthetic data)
    try:
        c0 = conn.cursor()
        c0.execute("SET synchronous_commit TO OFF")
        conn.commit()
    except Exception:
    ...
    try:
        ...
        total_sales = generate_sales(
            ...
            growth_store_id=args.growth_store_id,
            seasonal_months_arg=args.seasonal_months,
            seasonal_products_count=args.seasonal_products,
            seed=args.seed,
        )
